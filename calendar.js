class quickCalFrontEnd {
  ...
  globals = () => {
    ...
    this.amountOfDays = {
      ...
      feb: {
        month:
          (this.currentYearG % 4 == 0 && this.currentYearG % 100 != 0) ||
          this.currentYearG % 400 == 0
            ? 29
            : 28,
      },
      mar: { month: 31 },
  ...
  getCalendar = (date) => {
    ...
    var startOn = new Date(
      this.currentYearG,
      this.currentIndexOfMonthG,
      1
    ).getDay();

    //toggle set displayed -- might just stick in above function and run three separate classes considering things will probably change
    switch (this.displayType) {
      ...
      case "weekly":
        ...
        break;
      default:
        return; //not in set reset and re run
    }
    ...
    for (let i = 1; i <= days; i++) {
      ...
      th.onclick = () => {
        this.showForm(
          ...
          this.currentYearG,
          this.currentIndexOfMonthG,
          this.currentMonthNameG
        );
      };
      th.onmouseover = () => {
        this.addNumber(i);
        this.eliminateBookedEvents(
          i,
          this.currentYearG,
          this.currentIndexOfMonthG,
          this.currentMonthNameG
    ...
    document.getElementById("date").innerText = `
        ${this.currentIndexOfMonthG + 1}/${
      this.currentYearG
    } | ${this.currentMonthNameG.toUpperCase()} ${this.currentDayG} ${
      this.currentYearG
    }`;

    this.fileToGetBooked
      ? this.loadInTakenTimes()
      : this.showNoLoad("loading data not set.."); //doing this twice
    this.hideBackButton
      ? this.hideBackButtonNone()
      : this.showNoLoad("back button showing on same month and year..");
    this.hidePastDays
      ? this.hidePassedDaysNone()
      : this.showNoLoad("passed days not showing on month and year..");
  };
  ...
  next = () => {
    this.currentIndexOfMonthG === 11
      ? ((this.currentYearG += 1), (this.currentIndexOfMonthG = 0))
      : (this.currentIndexOfMonthG += 1);
  ...
  back = () => {
    this.currentIndexOfMonthG === 0
      ? ((this.currentYearG -= 1), (this.currentIndexOfMonthG = 11))
      : (this.currentIndexOfMonthG -= 1);
    this.getCalendar(new Date(this.currentYearG, this.currentIndexOfMonthG, 1));
  };
  ...
  hideBackButtonNone = () => {
    if (
      this.currentYearG <= this.todaysDate().year &&
      this.currentIndexOfMonthG <= this.todaysDate().month
    ) {
  ...
  //hide passed days
  hidePassedDaysNone = () => {
    //consider doing this on the fly when loaded in...just "continue" inside of loop
    if (
      this.currentYearG == this.todaysDate().year &&
      this.currentIndexOfMonthG == this.todaysDate().month
    ) {
  ...
  showForm = (day, year, monthIndex, monthName) => {
    ...
    temp += `<p id = "timeErrorS" style = "color: red"></p>
            </div>
  ...
  submit = () => {
    ...
    if (message.value.length < 10) {
      count++;
      document.getElementById("messageErrorS").innerText =
        "Message must be more at least 10 characters";
    }
  ...
  //grab and push in
  showCheckMark = () => {
    //grab calendarId and replace with a a check mark... dont let them do anything else
  };
  ...
  //whenever you call new date refer to this
  todaysDate = () => {
    return {
  ...
  checkBeforeToday = (day, year, monthIndex, monthName) => {
    if (
      (monthIndex === this.todaysDate().month &&
        year === this.todaysDate().year &&
        day < this.todaysDate().day) ||
      (monthIndex < this.todaysDate().month &&
        year === this.todaysDate().year) ||
      year < this.todaysDate().year
    ) {
  ...
  //prevent overload paused from function that reads and displays... ajax option
